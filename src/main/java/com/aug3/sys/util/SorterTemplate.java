...
/**
 * This class was inspired by CGLIB, but provides a better QuickSort algorithm
 * without additional InsertionSort at the end. To use, subclass and override
 * the four abstract methods which compare and modify your data. Allows custom
 * swap so that two arrays can be sorted at the same time.
 * 
 * @lucene.internal
 */
public abstract class SorterTemplate {
	...
	private static final int TIMSORT_STACKSIZE = 40; // change if you change
														// TIMSORT_MINRUN
	...
	/**
	 * Implement this method, that swaps slots {@code i} and {@code j} in your
	 * data
	 */
	protected abstract void swap(int i, int j);

	/**
	 * Compares slots {@code i} and {@code j} of you data. Should be implemented
	 * like <code><em>valueOf(i)</em>.compareTo(<em>valueOf(j)</em>)</code>
	 */
	protected abstract int compare(int i, int j);

	/**
	 * Implement this method, that stores the value of slot {@code i} as pivot
	 * value
	 */
	...
	/**
	 * Sorts via stable in-place BinarySort algorithm (O(n<sup>2</sup>)) (ideal
	 * for small collections which are in random order).
	 */
	...
	private class TimSort {
		...
		/**
		 * Compute the length of the next run, make the run sorted and return
		 * its length.
		 */
		int nextRun() {
	 ...
	 * >TimSort</a>, see also <a
	 * href="http://svn.python.org/projects/python/trunk/Objects/listobject.c"
	 * >source code</a>. TimSort is a stable sorting algorithm based on
	 * MergeSort but known to perform extremely well on partially-sorted inputs.
	...
	/**
	 * Sorts via stable in-place MergeSort algorithm For small collections falls
	 * back to {@link #insertionSort(int,int)}.
	 */
	...
	private void runMerge(int lo, int pivot, int hi, int len1, int len2) {
		...
		if (comparePivot(pivot) <= 0) {
			// all values from the first run are below all values from the 2nd
			// run
